...
# Вставь сюда свой Telegram user ID (целое число)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in user_templates:
        user_templates[user_id] = "HIMARS-1D-Gm80A"  # шаблон по умолчанию
    ...
    await update.message.reply_text(
        f"Выбери, сколько ключей нужно.\n\nТекущий шаблон: *{user_templates[user_id]}*",
        parse_mode="Markdown",
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_authorized(user_id):
        await query.answer()
        await query.message.reply_text("⛔️ Доступ запрещён.")
        return
